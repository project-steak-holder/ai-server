...
class SentimentService:
    ...
    @wide_event("apply_delta")
    async def apply_delta(self, conversation_id: str, raw_delta) -> None:
        """
        Retrieve current sentiment, apply delta, clamp to [-10, 10], and persist updated value.
        """
        sentiment_obj = await self.get_sentiment(conversation_id)
        if sentiment_obj and sentiment_obj.sentiment is not None:
            original_sentiment = Decimal(str(sentiment_obj.sentiment))
        else:
            original_sentiment = Decimal("0.00")
        try:
            delta = (
                Decimal(str(raw_delta)) if raw_delta is not None else Decimal("0.00")
            )
        except (InvalidOperation, ValueError):
            delta = Decimal("0.00")
        updated_sentiment = original_sentiment + delta
        updated_sentiment = max(
            Decimal("-10.00"), min(Decimal("10.00"), updated_sentiment)
        )
        await self.update_sentiment(conversation_id, updated_sentiment)
        add_event_context(
            current_sentiment=float(original_sentiment),
